...
const Button = ({text, children, disabled, backgroundColor, className, onClick}: ButtonProps) => {  
        return (
            <button disabled={disabled} onClick={onClick} className={` ${className} md:text-base font-bold text-sm w-fit rounded-lg pt-1 pl-2 pr-2 pb-1 flex items-center gap-2 hover:cursor-pointer  ${disabled ? "bg-grey hover:cursor-default" : backgroundColor } `}>
                {children}
                {text}
            </button>
        );
